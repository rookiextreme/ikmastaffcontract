...
<table>
    ...
    <tr>
        ...
        <td>{{ $pyd->name ?? '-' }}</td>
        <td>Dihantar</td>
        <td>{{ optional($evaluation->submitted_at)->format('d/m/Y h:i A') ?? '-' }}</td>
    </tr>
    <tr>
        <td>PPP</td>
        <td>{{ $ppp->name ?? '-' }}</td>
        <td>Dinilai</td>
        <td>{{ optional($evaluation->ppp_reviewed_at)->format('d/m/Y h:i A') ?? '-' }}</td>
    </tr>
    <tr>
        <td>PPK</td>
        <td>{{ $ppk->name ?? '-' }}</td>
        <td>Disahkan</td>
        <td>{{ optional($evaluation->ppk_approved_at)->format('d/m/Y h:i A') ?? '-' }}</td>
    </tr>
    <tr>
        <td>Admin / UPSM</td>
        <td>{{ optional($evaluation->logs->where('action', 'ADMIN_FINALIZE')->last()?->actor)->name ?? '-' }}</td>
        <td>Muktamad</td>
        <td>{{ optional($evaluation->logs->where('action', 'ADMIN_FINALIZE')->last()?->created_at)->format('d/m/Y h:i A') ?? '-' }}</td>
    </tr>
</table>
...
<table>
    ...
    <tbody>
        @forelse($evaluation->logs as $log)
            <tr>
        ...
        @empty
            <tr>
                <td colspan="4" style="text-align:center;">Tiada log.</td>
            </tr>
        @endforelse
    </tbody>
</table>
