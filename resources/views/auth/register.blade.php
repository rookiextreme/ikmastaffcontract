        ...
        <div class="d-flex flex-column flex-lg-row-fluid w-lg-50 p-10 order-2 order-lg-1">
            <div class="d-flex flex-center flex-column flex-lg-row-fluid">
                <div class="w-lg-500px p-10">
                    <form action="{{ route('register') }}" class="form w-100" method="post">
                        @csrf
                        <div class="text-center mb-11">
                            <h1 class="text-gray-900 fw-bolder mb-3">Selamat Datang <br>Sistem Pengurusan Staf Kontrak IKMa</h1>
                            <div class="text-gray-500 fw-semibold fs-6">Sila Masukkan Maklumat Untuk Mendaftar</div>
                        </div>
                        <div class="fv-row mb-3">
                            <input type="text" placeholder="Nama" name="name" autocomplete="off" class="form-control bg-transparent" value="{{ old('name') }}" required/>
                        </div>
                        <div class="fv-row mb-3">
                            <input type="text" placeholder="E-mel" name="email" autocomplete="off" class="form-control bg-transparent" value="{{ old('email') }}" required/>
                        </div>
                        <div class="fv-row mb-3">
                            <input type="text" placeholder="No. Kad Pengenalan" name="identification_no" autocomplete="off" class="form-control bg-transparent" value="{{ old('identification_no') }}" required/>
                        </div>
                        <div class="fv-row mb-3">
                            <input type="text" placeholder="No. Telefon" name="mobile_phone" autocomplete="off" class="form-control bg-transparent" value="{{ old('mobile_phone') }}" required/>
                        </div>
                        <div class="fv-row mb-3">
                            <input type="password" placeholder="Kata Laluan" name="password" autocomplete="off" class="form-control bg-transparent" required/>
                        </div>
                        <div class="fv-row mb-3">
                            <input type="password" placeholder="Konfirmasi Kata Laluan" name="password_confirmation" autocomplete="off" class="form-control bg-transparent" required/>
                        </div>
                        @if(isset($agent))
                    ...
                    @if(!empty($errors))
                        ...
                        @endforeach
                        @foreach($errors->get('identification_no') as $e)
                            <div class="text-center text-danger fw-bold">
                                {{ $e }}
                            </div>
                        @endforeach
            ...
            <div class="p-10 text-center gap-5 fw-semibold ">
                Sudah Ada Akaun?<a class="m-4 text-primary" href="{{ route('login') }}" >Sila Log Masuk</a><br><br>
            </div>
        </div>
