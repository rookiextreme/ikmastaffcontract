                ...
                <div class="row">
                    @if($staff->getStaffPosition->branch_position_id)
                        <div class="col-md-12 mb-4 vals-row mt-4 text-uppercase">
                            ...
                            {{-- ✅ Unit current (jika ada) --}}
                            @if(optional($staff->getStaffPosition->getBranchPosition)->unit_id)
                                <br>
                                <span class="text-primary">
                                    {{ optional($staff->getStaffPosition->getBranchPosition->getUnit)->name ?? '' }}
                                </span>
                            @endif
                        </div>
                    @endif
                    ...
                    <div class="col-md-3 mb-4 vals-row mt-4">
                        ...
                        <select class="form-control" id="state-select" data-control="select2">
                            ...
                            @foreach($state as $s)
                                <option value="{{ $s->id }}" {{ $state_select ? ($state_select == $s->id ? 'selected' : '') : '' }}>
                                    {{ $s->name }}
                                </option>
                            @endforeach
                        </select>
