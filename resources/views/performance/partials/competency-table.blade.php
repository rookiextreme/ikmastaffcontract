{{-- resources/views/performance/partials/competency-table.blade.php --}}
...
@php
    ...
    // LOCKING (ikut flow)
    // =========================
$lockedPPP = !in_array($status, ['SUBMITTED'], true); // ✅ PPP hanya boleh edit masa SUBMITTED sahaja
    $lockedPPK = !in_array($status, ['PPP_SCORED', 'PPK_APPROVED'], true);
    ...
    // =========================
    // NORMALIZE NAME (supaya mapping match)
...
<table class="table table-bordered align-middle">
    ...
    @foreach($items as $idx => $item)
        @php
            ...
            $ppkVal = old("ppk_scores.{$item->id}.score", $row->ppk_score ?? '');
        ...
        <tr>
            ...
            <td class="text-center">
                @if($canEditPPP)
                    {{-- BAHAGIAN VI: boleh kosong jika PYD tiada catatan. Jadi min tak paksa. --}}
                    <input type="number"
                           min="{{ $bahagian === 'VI' ? 0 : 1 }}"
                           max="10"
                           class="form-control text-center"
                           name="ppp_scores[{{ $item->id }}][score]"
                           value="{{ $pppVal }}">
                @else
